...
class ImageSequence(keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        for i in range(self.batch_size):
            ...
            img = cv2.imread(os.path.join(self.directory_name, random_image_file))
            img = cv2.medianBlur(img, 3)

            grayImage = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
            try1 = cv2.adaptiveThreshold(grayImage, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 3, 0.0002)
            try1 = cv2.medianBlur(try1,3)

            processed_data = numpy.array(try1)
